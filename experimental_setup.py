                ...
                for e, exp in enumerate(experiments):
                    ...
                    try:  
                        ...
                        if exp=='Evolved_Weights_Holdout':
                            # Split the training set into training and validation sets
                            X_train, X_val, y_train, y_val = train_test_split(X_train_val, y_train_val, test_size=0.2, stratify=y_train, random_state=r)
                            ...
                            ga_func = partial(utils.fitness_func_holdout, model = ml(random_state=super_seed), X_train=X_train, y_train=y_train, X_val=X_val, y_val=y_val, sens_features=sens_features, objective_fuctions=['accuracy', 'subgroup_fnr'], objective_functions_weights=[1, -1])
                            ga_func.__name__ = 'ga_func'
                            ga = ga.GA(ind_size = 2**(len(sens_features)+ 1), random_state=super_seed, fitness_func=ga_func, **ga_params)
                            ga.optimize()
                            ...
                            for j in range(ga.evaluated_individuals.shape[0]):
                                ...
                                train_score = utils.evaluate_objective_functions(est, X_val, y_val, ['accuracy', 'subgroup_fnr'],sens_features)
                                test_score = utils.evaluate_objective_functions(est, X_test, y_test, ['accuracy', 'subgroup_fnr'], sens_features)
                            ...
                            X_train, X_select, y_train, y_select = utils.learn_sel_fair_split(X_train_val, y_train_val, sens_features, r, 0.2)
                            print("Training data: ", X_train, y_train)

                            scores = pd.DataFrame(columns = ['taskid','exp_name','seed','run','auroc', 'accuracy', 'balanced_accuracy', 'fairness', 'train_auroc', 'train_accuracy', 
                                                            'train_balanced_accuracy', 'train_fairness'])
                            ga_func = partial(utils.fitness_func_lexidate, model = ml(random_state=super_seed), X_learn=X_train, y_learn=y_train,  X_select=X_select, y_select=y_select, sens_features=sens_features)
                            ga_func.__name__ = 'ga_func'
                            ga = ga.GA(ind_size = 2**(len(sens_features)+ 1), random_state=super_seed, fitness_func=ga_func, **ga_params)
                            ga.optimize()
