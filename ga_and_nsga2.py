# intending to merge ga.py and ga_nsga2.py files for simplicity
...
class GA():
    ...
    def mutation(self, ind):
        ...
        if self.use_nsga:
            # NSGA-II: Check mutation probability for each element
            ...
            # Standard GA: Check mutation probability once
            if self.rng.random() < self.mut_rate:
                for i in range(len(_program)):
                    if self.rng.random() < 0.5:
                        new_val = _program[i] + self.rng.random()
    ...
    def selection(self):
        ...
        while len(cases) > 0 and len(candidates) > 1:
            ...
            # Only remove the current case in GA
            if not self.use_nsga:
                cases.pop(0)
    ...
    def evaluate_population(self, pop=None):
        ...
        if not self.use_nsga:
            # Update the best individual for GA
            candidates = self.population
            cases = list(range(len(self.population[0].fitness)))
    ...
    def optimize(self):
        ...
        if not self.use_nsga:
            # Evaluate the initial population for GA
            self.evaluate_population()
